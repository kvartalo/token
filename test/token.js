...
contract("token", (accounts) => {
    ...
    async function transferoff (fromaddr, frompvk, toaddr, value) {
        let nonce = (await token.nonces(fromaddr)).toNumber()
        ...
        await token.transferoff(fromaddr,toaddr,value,sig.r,sig.s,sig.v)
    } 
    ...
    it("can transfer using offchain signatures" , async() => {
        ...
        await transferoff(addr1,pvk1,addr2,500);
        assert.equal(500,await token.balanceOf(addr1));
